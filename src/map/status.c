	...
	**/
	...
	if(flag&SCB_FLEE) {
		...
		if (status->agi == b_status->agi) {
			status->flee = status_calc_flee(bl, sc, b_status->flee, false);
			displaystatus->flee = status_calc_flee(bl, sc, b_status->flee, true);
		} else {
			flee = b_status->flee +(status->agi - b_status->agi, false);
			status->flee = status_calc_flee(bl, sc, flee, false);
			display_status->flee = status_calc_flee(bl, sc, flee, true);
		}
#endif
	...
	if(flag&SCB_MATK) {
		...
		switch( bl->type ) {
			case BL_PC: {
				...
				status->matk_min = status_calc_ematk(bl, sc, status->matk_min, false);
				display_status->matk_min = status_calc_ematk(bl, sc, status->matk_min, true);
				status->matk_max = status->matk_min;
				display_status->matk_max = display_status->matk_min;
		...
		status->matk_max = status_calc_matk(bl, sc, status->matk_max, false);
		status->matk_min = status_calc_matk(bl, sc, status->matk_min, false);
		display_status->matk_max = status_calc_matk(bl, sc, status->matk_max, true);
		display_status->matk_min = status_calc_matk(bl, sc, status->matk_min, true);
	}
